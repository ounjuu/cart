...
const dataAll3 = () => {
  ...
  // 좋아요 데이터
  const getDate3 = JSON.parse(localStorage.getItem("userLike") || "[]");
  const getDate4 = JSON.parse(localStorage.getItem("userInfo") || "[]");
  ...
  const commonItems = data.filter((item1) =>
    likeData.some((item2) => item1.id === item2.id)
  );

  // 중복된 id를 제거
  const uniqueCommonItems = Array.from(
    new Set(commonItems.map((item) => item.id))
  ).map((id) => commonItems.find((item) => item.id === id));

  console.log(uniqueCommonItems, "??");

  products_wrap.innerHTML = uniqueCommonItems
    .map((x, i) => {
